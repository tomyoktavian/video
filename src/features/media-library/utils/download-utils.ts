...
function getDownloadWorker(): Worker {
  if (!downloadWorker) {
    downloadWorker = new Worker(new URL('../workers/media-download-worker.ts', import.meta.url), {
      type: 'module',
    })
  }
...
export function buildEpisodeFileName(ep: OnlineEpisode, originalIndex: number): string {
