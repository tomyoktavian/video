...
import { Video, FileAudio, Image as ImageIcon, Trash2, Loader2, Link2Off, RefreshCw, Zap, FileText, Play, Square, Sparkles, Download, CheckCircle2, ListPlus } from 'lucide-react';
import {
  ContextMenu,
  ContextMenuContent,
  ContextMenuItem,
...
import {
  useItemsStore, useTimelineStore, resolveMediaUrl,
  buildDroppedMediaTimelineItem, getDroppedMediaDurationInFrames,
  addItem as timelineAddItem, addItems as timelineAddItems,
} from '../deps/timeline';
...
export interface OnlineEpisode {
  ...
  duration?: number;
  thumbnail?: string;
  /** Root-level drama/video title from the imported JSON */
  dramaTitle?: string;
  date?: string;
  size?: number;
...
export type FetchStatus = "queued" | "fetching" | "done" | "error";

interface MediaCardProps {
  media?: MediaMetadata;
  onlineEpisode?: OnlineEpisode;
  onlineFetchStatus?: FetchStatus;
  isOnlineDownloaded?: boolean;
  isOnlineDownloading?: boolean;
  onOnlineDownload?: () => void;
  onlineDownloadProgress?: number | null;
  onlineIndex?: number;
  selected?: boolean;
  isBroken?: boolean;
...
function MediaCardActionMenuItems({
  ...
  const showTranscriptGroup = canShowGenerateTranscript || canShowDeleteTranscript || (hasTranscript && !isBroken && !isTranscribing);
  const showAiGroup = isTaggable && !isBroken && !isTagging;

  const groups: ReactNode[] = [];

  if (!isBroken && onAddToTimeline) {
    groups.push(
  ...
  if (showTranscriptGroup) {
    groups.push(
      <Fragment key="transcript">
        ...
        {hasTranscript && !isBroken && !isTranscribing && (
          <ContextMenuItem onClick={onShowTranscript}>
            <FileText className="w-3 h-3 mr-2" />
            Show Transcript
          </ContextMenuItem>
        )}
...
export const MediaCard = memo(function MediaCard({
  ...
  onlineEpisode,
  onlineFetchStatus,
  isOnlineDownloaded = false,
  isOnlineDownloading = false,
  onOnlineDownload,
  ...
  const isOnline = !!onlineEpisode;
  const mediaId = isOnline ? '' : (media?.id ?? '');
  const fileName = isOnline ? `${onlineIndex + 1}-${onlineEpisode?.dramaTitle || 'episode'}` : (media?.fileName ?? '');
  const duration = isOnline ? (onlineEpisode?.duration || 0) : (media?.duration ?? 0);
  const mediaType = isOnline ? 'video' : getMediaType(media?.mimeType ?? '');
  ...
  const proxyStatus = useMediaLibraryStore((s) => isOnline ? undefined : s.proxyStatus.get(mediaId));
  const transcriptStatus = useMediaLibraryStore((s) => isOnline ? 'idle' : s.transcriptStatus.get(mediaId) ?? 'idle');
  const transcriptProgress = useMediaLibraryStore((s) => isOnline ? undefined : s.transcriptProgress.get(mediaId));

  const isTranscribable = !isOnline && (mediaType === 'video' || mediaType === 'audio');
  const canGenerateProxy =
    !isOnline && mediaType === 'video'
    && !isBroken
    && !isImporting
    && !!media?.mimeType
  ...
  const isTagging = useMediaLibraryStore((s) => !isOnline && s.taggingMediaIds.has(mediaId));
  ...
  useEffect(() => {
    ...
    if (isOnline) {
      setThumbnailUrl(onlineEpisode?.thumbnail || null);
      return;
    }
  ...
  }, [isOnline, isOnline ? onlineEpisode?.thumbnail : media?.id, isOnline ? undefined : media?.thumbnailId]);

  const getTargetMediaItems = useCallback((): MediaMetadata[] => {
    if (isOnline || !media) return [];
    ...
    return [media];
  }, [media, isOnline, mediaId]);

  const handleContextMenuOpenChange = useCallback((open: boolean) => {
    if (!open) return;
    const store = useMediaLibraryStore.getState();
    if (!isOnline && !store.selectedMediaIds.includes(mediaId)) { store.setSelection({ mediaIds: [mediaId], compositionIds: [] }); }
  }, [mediaId]);
  ...
  const handleShowTranscript = useCallback((e: React.MouseEvent) => {
    e.preventDefault();
    e.stopPropagation();
    if (!media) return;
    useTranscriptViewerDialogStore.getState().open(media.id, media.fileName);
  ...
  const handleAddToTimeline = useCallback(async (e: React.MouseEvent) => {
    ...
    const showNotification = useMediaLibraryStore.getState().showNotification;
    ...
    try {
      ...
      // Calculate end frame of the last item in the timeline
      const items = useItemsStore.getState().items;
      let endFrame = 0;
      for (const item of items) {
        const itemEnd = item.from + item.durationInFrames;
        if (itemEnd > endFrame) endFrame = itemEnd;
      }

      // Get canvas size from project
      ...
      const durationInFrames = getDroppedMediaDurationInFrames(media, mediaType as 'video' | 'audio' | 'image', fps);

      // Find the first compatible track for the media type, or use the first available
      const tracks = useItemsStore.getState().tracks;
      const trackKindForMedia = mediaType === 'audio' ? 'audio' : 'video';
      ...
      // Build linked video+audio items if the video has audio
      ...
      if (hasAudio) {
        ...
        if (audioTrack) {
          const audioItem = buildDroppedMediaTimelineItem({
            media,
            mediaId: media.id,
            mediaType: 'audio',
            label: media.fileName,
          ...
          timelineAddItems([primaryItem, audioItem]);
        } else {
      ...
      useMediaLibraryStore.getState().showNotification({ type: 'error', message: 'Failed to add media to timeline' });
    }
  }, [media, mediaType]);
  ...
  }, [mediaId, fileName, duration, mediaType]);
  ...
  const updateSkimPreview = useCallback((clientX: number) => {
    ...
    const durationInFrames = Math.max(1, Math.round(duration * (media?.fps || 30)));
    const frame = Math.min(durationInFrames - 1, Math.max(0, Math.round(progress * (durationInFrames - 1))));

    setSkimProgress(progress);
    setMediaSkimPreview(mediaId, frame);
  }, [canHoverPreview, canScrubPreview, duration, media?.fps, mediaId, setMediaSkimPreview]);
  ...
  useEffect(() => {
    ...
    return () => {
      cancelScheduledSkimPreview();
      if (useEditorStore.getState().mediaSkimPreviewMediaId === mediaId) {
        clearMediaSkimPreview();
      }
    };
  }, [canHoverPreview, cancelScheduledSkimPreview, clearMediaSkimPreview, mediaId]);
  ...
  }, [audioPlaying, mediaId]);

  const handleSeekToCaption = useCallback((timeSec: number) => {
    const fps = media?.fps || 30;
    const sourceDurationFrames = Math.max(1, Math.round(duration * fps));
  ...
  const actionMenuItems = isOnline ? (
    <>
      <ContextMenuItem onClick={(e) => { e.stopPropagation(); onOnlineDownload?.(); }} disabled={isOnlineDownloaded || onlineFetchStatus !== "done"}>
    ...
    return (
      ...
      >
        ...
        >
          ...
          {/* Online downloaded indicator for list view */}
          {isOnline && isOnlineDownloaded && !isOnlineDownloading && (
            <div className="absolute top-0.5 right-0.5 p-0.5 rounded bg-green-500/90 text-white pointer-events-none">
              <CheckCircle2 className="w-2 h-2" />
            </div>
          )}
          {/* Online downloading progress for list view */}
          {isOnline && isOnlineDownloading && (
            <>
              ...
              <div className="absolute bottom-0 left-0 right-0 h-[2px] bg-black/30 pointer-events-none">
                <div
                  ...
                  style={{ width: `${onlineDownloadProgress ?? 0}%` }}
                />
              </div>
            </>
          )}
      ...
      >
        ...
        {!isImporting && (
          <div className="absolute top-1 right-1 z-10 flex flex-col items-end gap-0.5">
            ...
            {isOnline && isOnlineDownloaded && !isOnlineDownloading && (
              <div className="p-0.5 rounded bg-green-500/90 text-white pointer-events-none" title="Downloaded">
        ...
        {/* Online downloading progress bar for grid view */}
        {isOnline && isOnlineDownloading && (
          <>
            <div className="absolute inset-0 bg-black/40 flex flex-col items-center justify-center gap-1.5 pointer-events-none">
