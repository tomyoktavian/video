  ...
  if (isImageMedia && !isBroken && onSetCoverToCompounds) {
    groups.push(
      <Fragment key="compound-cover">
        <ContextMenuLabel>{t('media.card.menuCompound')}</ContextMenuLabel>
        <ContextMenuItem onClick={onSetCoverToCompounds}>
          <Sparkles className="w-3 h-3 mr-2" />
          {t('media.card.setCoverToCompound')}
        </ContextMenuItem>
      </Fragment>,
    )
  }
...
export const MediaCard = memo(function MediaCard({
  ...
  const fileName = isOnline
    ? `${onlineIndex + 1}-${onlineEpisode?.dramaTitle || 'episode'}`
    : (media?.fileName ?? '')
  ...
  const transcriptProgress = useMediaLibraryStore((s) =>
    isOnline ? undefined : s.transcriptProgress.get(mediaId),
  )
  ...
  const hasProxy = useMediaLibraryStore((s) =>
    isOnline ? false : s.proxyStatus.get(mediaId) === 'ready',
  )
  const proxyStatus = useMediaLibraryStore((s) =>
    isOnline ? undefined : s.proxyStatus.get(mediaId),
  )
  ...
  const mediaThumbnailId = media?.thumbnailId
  ...
  const handleContextMenuOpenChange = useCallback(
    (open: boolean) => {
      if (!open) return
      const store = useMediaLibraryStore.getState()
      if (!isOnline && !store.selectedMediaIds.includes(mediaId)) {
        store.setSelection({ mediaIds: [mediaId], compositionIds: [] })
      }
    },
    [mediaId, isOnline],
  )
  ...
  useEffect(() => {
    ...
    if (isOnline) {
      setThumbnailUrl(episodeThumbnail || null)
      return
    }
  ...
  }, [isOnline, episodeThumbnail, mediaIdAttr, mediaThumbnailId, media, mediaId])
  ...
  const handleDragStart = useCallback(
    ...
    [installNativeDragCleanupListeners, mediaId, fileName, duration, mediaType],
  )
  ...
  const actionMenuItems = isOnline ? (
    <>
      <ContextMenuItem
        ...
        disabled={isOnlineDownloaded || onlineFetchStatus !== 'done'}
      >
              ...
              <div className="flex-1 min-w-0 flex items-center gap-1.5">
                {isImporting ? (
                  ...
                  <>
                    ...
                    <h3 className="text-xs font-medium text-foreground truncate">{fileName}</h3>
    ...
    <>
      ...
      `}
          ...
          >
            ...
            <div className="absolute top-0 left-0 right-0 h-1 bg-linear-to-r from-secondary via-muted to-secondary" />
            <div className="absolute bottom-0 left-0 right-0 h-1 bg-linear-to-r from-secondary via-muted to-secondary" />
            ...
            >
              ...
              {!isPreparingMedia && (
                <div className="absolute top-1 right-1 z-10 flex flex-col items-end gap-0.5">
                  ...
                  {isOnline && isOnlineDownloaded && !isOnlineDownloading && (
                    <div
                      className="p-0.5 rounded bg-green-500/90 text-white pointer-events-none"
                      title="Downloaded"
                    >
