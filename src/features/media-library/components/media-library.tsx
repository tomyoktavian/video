...
import { Search, Filter, SortAsc, Video, FileAudio, Image as ImageIcon, Trash2, Grid3x3, List, AlertTriangle, Info, X, FolderOpen, Link, Link2Off, ChevronRight, Film, ArrowLeft, Zap, Loader2, Copy, Check, Upload, Sparkles, FileText, ScanSearch, CloudDownload, ListPlus, MoreHorizontal, Merge } from 'lucide-react';
...
import { OnlineMediaTab } from './online-media-tab';
...
import { ffmpegMergeService, type MergeProgressEvent } from '../services/ffmpeg-merge-service';
...
import { addItems as timelineAddItems } from '@/features/media-library/deps/timeline-actions';
import {
...
export const MediaLibrary = memo(function MediaLibrary({ onMediaSelect }: MediaLibraryProps) {
  ...
  // Count selected video items eligible for merge
  const selectedVideoCount = useMemo(() => {
    return selectedMediaIds.filter((id) => {
      const m = mediaById[id];
      return m && m.mimeType.startsWith('video/');
    }).length;
  }, [selectedMediaIds, mediaById]);

  // Preload FFmpeg engine when 2+ videos are selected (likely to merge)
  useEffect(() => {
    if (selectedVideoCount >= 2) {
      ffmpegMergeService.preload().catch(() => {
        // Best effort — merge will load on demand if preload fails
      });
    }
  }, [selectedVideoCount]);

  const handleMergeSelected = useCallback(async () => {
    if (selectedVideoCount < 2 || isMergingRef.current) return;
    isMergingRef.current = true;

    const store = useMediaLibraryStore.getState();
    const projectId = store.currentProjectId;
    if (!projectId) {
      store.showNotification({ type: 'error', message: 'No project selected' });
      isMergingRef.current = false;
      return;
    }

    const videoItems = selectedMediaIds
      .map((id) => mediaById[id])
      .filter((m): m is MediaMetadata =>
        m !== undefined && m.mimeType.startsWith('video/')
      );
    ...
    try {
      setMergeProgress({ status: 'loading', text: 'Menyiapkan video files…', progress: 0 });

      const files: Array<{ blob: Blob; name: string; videoCodec?: string; audioCodec?: string; mimeType?: string }> = [];
      for (let i = 0; i < videoItems.length; i++) {
        const item = videoItems[i]!;
        setMergeProgress({
          status: 'loading',
          text: `Membaca ${i + 1}/${videoItems.length}: ${item.fileName}`,
          progress: Math.round((i / videoItems.length) * 30),
        });
        const blob = await mediaLibraryService.getMediaFile(item.id);
        if (!blob) throw new Error(`Gagal membaca file: ${item.fileName}`);
        files.push({
          blob,
          name: item.fileName,
      ...
      const mergedBlob = await ffmpegMergeService.mergeVideos(files, (evt) => {
        const now = Date.now();
        if (evt.status === 'done' || evt.status === 'error' || now - lastProgressUpdate >= 250) {
          lastProgressUpdate = now;
          setMergeProgress(evt);
        }
      });

      const firstName = videoItems[0]!.fileName.replace(/\.[^.]+$/, '');
      const lastName = videoItems[videoItems.length - 1]!.fileName.replace(/\.[^.]+$/, '');
      const mergedFileName = `${firstName}_to_${lastName}_merged.mp4`;

      setMergeProgress({ status: 'merging', text: 'Mengimpor hasil merge ke library…', progress: 98 });

      // Import merged result as a new media item via OPFS
      // Use importOnlineMedia directly (not importMediaFromUrl) because
      // blob: URLs are rejected by the URL validator in importMediaFromUrl.
      const mergedUrl = URL.createObjectURL(mergedBlob);
      try {
        await mediaLibraryService.importOnlineMedia(
          mergedUrl,
          mergedFileName,
          projectId,
        );
      ...
      setMergeProgress({ status: 'done', text: 'Merge selesai!', progress: 100 });
      store.showNotification({
        type: 'success',
        message: `Berhasil merge ${videoItems.length} video → ${mergedFileName}`,
      });
      setTimeout(() => setMergeProgress(null), 3000);
    } catch (error) {
      logger.error('Merge selected videos failed:', error);
      setMergeProgress({
        status: 'error',
      ...
      store.showNotification({
        type: 'error',
        message: `Merge gagal: ${error instanceof Error ? error.message : String(error)}`,
      });
    } finally {
      isMergingRef.current = false;
    }
  }, [selectedMediaIds, mediaById, selectedVideoCount]);

  const totalMediaCount = filteredMediaItems.length;
  const allLocalSelected = totalMediaCount > 0 && selectedMediaIds.length === totalMediaCount;
  const someLocalSelected = selectedMediaIds.length > 0 && selectedMediaIds.length < totalMediaCount;

  const handleSelectAll = useCallback(() => {
    if (allLocalSelected) {
      clearSelection();
    } else {
      const allIds = filteredMediaItems.map((m) => m.id);
      setSelection({ mediaIds: allIds, compositionIds: [] });
    }
  }, [allLocalSelected, clearSelection, filteredMediaItems, setSelection]);

  const handleAddSelectedToTimeline = useCallback(async () => {
    if (selectedMediaIds.length === 0) return;

    const store = useMediaLibraryStore.getState();
    const allMedia = store.mediaItems;
    ...
    try {
      ...
      const avTrack =
        tracks.find((t: import('@/types/timeline').TimelineTrack) => t.kind === 'video') ?? tracks[0];
      const audioTrack = tracks.find(
        (t: import('@/types/timeline').TimelineTrack) => t.kind === 'audio',
      );

      if (!avTrack) {
        store.showNotification({ type: 'warning', message: 'No tracks available in the timeline' });
        return;
      }

      const allTimelineItems: import('@/types/timeline').TimelineItem[] = [];
      let currentFrame = endFrame;

      for (const media of selectedMedia) {
        const mType = getMediaType(media.mimeType) as 'video' | 'audio' | 'image';
        const blobUrl = await resolveMediaUrl(media.id);
        if (!blobUrl) continue;

        const needsThumbnail = mType === 'video' || mType === 'image';
        const thumbUrl = needsThumbnail
          ? await mediaLibraryService.getThumbnailBlobUrl(media.id)
          : null;

        const durationInFrames = getDroppedMediaDurationInFrames(media, mType, fps);
        const targetTrack = mType === 'audio' ? (audioTrack ?? avTrack) : avTrack;
        const hasAudio = mType === 'video' && !!media.audioCodec;
        ...
        const primaryItem = buildDroppedMediaTimelineItem({
          ...
          canvasWidth,
          canvasHeight,
          placement: { trackId: targetTrack.id, from: currentFrame, durationInFrames },
          originId,
          linkedGroupId,
        });
        allTimelineItems.push(primaryItem);
        ...
        if (hasAudio && audioTrack) {
          const audioItem = buildDroppedMediaTimelineItem({
            ...
            canvasWidth,
            canvasHeight,
            placement: { trackId: audioTrack.id, from: currentFrame, durationInFrames },
            originId,
            linkedGroupId,
          });
          allTimelineItems.push(audioItem);
        }
      ...
      if (allTimelineItems.length > 0) {
        timelineAddItems(allTimelineItems);
        store.showNotification({
          type: 'success',
          message: `Added ${selectedMedia.length} media to timeline`,
        });
      }
    } catch {
      useMediaLibraryStore.getState().showNotification({
        type: 'error',
        message: 'Failed to add selected media to timeline',
      });
    }
  }, [selectedMediaIds]);
  ...
  return (
    <div ref={containerRef} className="h-full flex flex-col">
      {/* Tab bar: Local / Online */}
      <div className="px-3 py-2 border-b border-border flex-shrink-0">
        <div className="flex bg-secondary p-0.5 rounded-md border border-border">
          <button
            onClick={() => setActiveTab('local')}
            className={cn(
              'flex items-center gap-1.5 h-6 px-2.5 rounded text-[11px] font-medium transition-colors',
              activeTab === 'local'
      ...
      <div className="px-4 pt-3 pb-2 space-y-2 flex-shrink-0">
        ...
        <div className="@container flex items-center gap-1.5 min-w-0">
          ...
          <HeaderActionTooltip label={allLocalSelected ? 'Deselect all' : 'Select all'}>
            ...
            >
              {(allLocalSelected || someLocalSelected) && (
        ...
        <BackgroundTaskProgress
          ...
          meta={(
            <span className="tabular-nums">
              {Math.round(mergeProgress.progress ?? 0)}%
            </span>
          )}
