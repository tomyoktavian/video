...
import { useItemsStore, resolveMediaUrl } from '@/features/media-library/deps/timeline-stores'
...
export const MediaLibrary = memo(function MediaLibrary({ onMediaSelect }: MediaLibraryProps) {
  ...
  const handleMergeSelected = useCallback(async () => {
    ...
    const videoItems = selectedMediaIds
      .map((id) => mediaById[id])
      .filter((m): m is MediaMetadata => m !== undefined && m.mimeType.startsWith('video/'))
    ...
    try {
      ...
      setMergeProgress({
        status: 'merging',
        text: 'Mengimpor hasil merge ke library…',
        progress: 98,
      })
      ...
      try {
        const { mediaLibraryService } = await importMediaLibraryService()
        await mediaLibraryService.importOnlineMedia(mergedUrl, mergedFileName, projectId)
      } finally {
  ...
  const handleAddSelectedToTimeline = useCallback(async () => {
    ...
    try {
      ...
      const avTrack =
        tracks.find((t: import('@/types/timeline').TimelineTrack) => t.kind === 'video') ??
        tracks[0]
      ...
      for (const media of selectedMedia) {
        ...
        const thumbUrl = needsThumbnail
          ? await importMediaLibraryService().then(({ mediaLibraryService }) =>
