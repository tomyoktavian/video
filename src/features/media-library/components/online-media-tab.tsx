...
async function extractVideoMetadata(url: string): Promise<{ duration: number; thumbnail: string }> {
...
export function OnlineMediaTab({
  ...
  const handleImportOnline = () => {
    try {
      ...
      if (parsed.episodes && Array.isArray(parsed.episodes)) {
        ...
        const episodesWithIndex = parsed.episodes.map((ep: OnlineEpisode, i: number) => {
  ...
  const handleSingleDownload = useCallback(
    async (ep: OnlineEpisode, originalIndex: number) => {
      ...
      const success = await store.startSingleDownload(ep, originalIndex, currentProjectId, {
        onSuccess: async () => {
  ...
  const handleDownloadAll = useCallback(async () => {
    ...
    const pendingEpisodes = sortedEpisodes.filter((ep) => ep.url && !downloadedUrls.has(ep.url))
    ...
    <div className="flex flex-1 flex-col h-full overflow-hidden p-4">
      {onlineEpisodes.length === 0 ? (
        <div className="flex flex-1 flex-col">
          <div className="mb-2 text-xs text-muted-foreground">Import JSON Data</div>
          <textarea
          ...
          <div className="mt-3 flex justify-end">
            <Button onClick={handleImportOnline} disabled={!jsonInput.trim()} size="sm">
              Parse JSON
            </Button>
          </div>
        </div>
        ...
        <div className="flex flex-1 flex-col overflow-hidden">
          ...
          <div className="custom-scrollbar flex-1 overflow-y-auto pr-1">
            ...
            >
              {sortedEpisodes.map((ep, i) => {
                ...
                const isDownloaded = ep.url ? downloadedUrls.has(ep.url) : false
                const isDownloading = ep.url ? downloadingUrls.has(ep.url) : false
                ...
                return (
                  <MediaCard
                    ...
                    onOnlineDownload={() => {
                      if (!isDownloaded && status === 'done' && ep.url && currentProjectId) {
                        handleSingleDownload(ep, originalIndex)
                      }
                    }}
                    onDelete={() => {
                      setOnlineEpisodes((prev) => prev.filter((_, idx) => idx !== originalIndex))
          ...
          {downloadProgress && (
            <div className="mt-2 rounded-md border border-border bg-secondary/50 px-3 py-2">
              <div className="flex items-center justify-between text-xs mb-1.5">
                <div className="flex items-center gap-1.5 min-w-0">
                  ...
                  <span className="text-muted-foreground shrink-0">
                    ({downloadProgress.episodeIndex + 1}/{downloadProgress.totalEpisodes})
                  </span>
                </div>
              ...
              {downloadProgress.total > 0 && downloadProgress.phase === 'downloading' && (
