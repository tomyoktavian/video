...
class MediaLibraryService {
  ...
  async importOnlineMedia(
    ...
    options?: {
      ...
      /** Pre-downloaded blob (from Web Worker). Skips the fetch step. */
      preDownloadedBlob?: Blob
    },
  ): Promise<MediaMetadata> {
    // Stage 1: Get the blob — either pre-downloaded or fetch now
    let blob: Blob

    if (options?.preDownloadedBlob) {
      blob = options.preDownloadedBlob
    } else {
    ...
    // Extract extension from URL pathname (not query params)
    let ext = 'mp4'
    try {
      ...
      if (dotIdx > 0) {
        const parsed = lastSegment.slice(dotIdx + 1).toLowerCase()
        if (parsed.length <= 5 && /^[a-z0-9]+$/.test(parsed)) {
          ext = parsed
        }
      }
    } catch {
      // Invalid URL, keep default 'mp4'
    }
    ...
    // Stage 2: Check for duplicates (by sourceUrl or fileName + fileSize)
    const projectMedia = await getMediaForProjectDB(projectId)
    const existingMedia = projectMedia.find(
      (m) => m.sourceUrl === url || (m.fileName === finalFileName && m.fileSize === file.size),
    )
    if (existingMedia) {
      return existingMedia
    }

    // Stage 3: Process media in worker (metadata + thumbnail)
    const { metadata, thumbnail } = await mediaProcessorService.processMedia(file, mimeType, {
      thumbnailTimestamp: 1,
    })

    // Stage 4: Save thumbnail
    const mediaId = crypto.randomUUID()
    let thumbnailId: string | undefined
    let thumbnailBlob = thumbnail

    // If worker didn't generate a thumbnail, try using the online thumbnail
    if (!thumbnailBlob && options?.thumbnailDataUrl) {
      try {
        const res = await fetch(options.thumbnailDataUrl)
    ...
    if (thumbnailBlob) {
      try {
        ...
        logger.warn('Failed to save online media thumbnail:', error)
        thumbnailId = undefined
      }
    }

    // Stage 5: Build metadata and persist to OPFS
    const opfsPath = buildGeneratedMediaOpfsPath(mediaId)
    const createdAt = Date.now()
    ...
    const mediaMetadata: MediaMetadata = {
      ...
      fileSize: file.size,
      mimeType,
      duration: 'duration' in metadata ? metadata.duration : options?.onlineDuration || 0,
      width: 'width' in metadata ? metadata.width : 0,
      height: 'height' in metadata ? metadata.height : 0,
      fps: metadata.type === 'video' ? metadata.fps : 30,
      codec:
