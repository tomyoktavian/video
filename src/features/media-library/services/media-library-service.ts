...
class MediaLibraryService {
  ...
  async importOnlineMedia(
    ...
    const existingMedia = projectMedia.find(
      (m) =>
        m.sourceUrl === url ||
        (m.fileName === finalFileName && m.fileSize === file.size),
    );
    if (existingMedia) {
    ...
    const mediaMetadata: MediaMetadata = {
      ...
      audioCodec: metadata.type === "video" ? metadata.audioCodec : undefined,
      audioCodecSupported:
        metadata.type === "video" ? metadata.audioCodecSupported : true,
      keyframeTimestamps:
        metadata.type === "video" ? metadata.keyframeTimestamps : undefined,
      gopInterval: metadata.type === "video" ? metadata.gopInterval : undefined,
      thumbnailId,
      sourceUrl: url,
      tags: [],
