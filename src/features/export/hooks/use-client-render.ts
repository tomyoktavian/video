...
export function useClientRender(): UseClientRenderReturn {
  ...
  const exportWorkerManagerRef = useRef<ReturnType<
    typeof createManagedWorker<Worker>
  > | null>(null);
  ...
  const startExport = useCallback(
    async (settings: ExportSettings | ExtendedExportSettings) => {
      ...
      try {
          ...
          if (!supportedCodecs.includes(clientSettings.codec)) {
            ...
            if (containerFallback) {
              ...
              clientSettings.container =
                getPreferredContainerForCodec(browserFallback);
        ...
        // Optimize bitrate: scale by codec efficiency and resolution
        if (exportMode === "video") {
          ...
          if (
            maxSourceBitrate > 0 &&
            clientSettings.videoBitrate > maxSourceBitrate * 1.3
          ) {
  ...
  const getSupportedCodecsForResolution = useCallback(
    async (options?: {
      ...
      const currentProject = useProjectStore.getState().currentProject;
      const width =
        options?.resolution?.width ?? currentProject?.metadata?.width ?? 1920;
      const height =
        options?.resolution?.height ?? currentProject?.metadata?.height ?? 1080;
      const bitrate =
        options?.bitrate ??
        (options?.quality
          ? getVideoBitrateForQuality(options.quality)
          : undefined);
