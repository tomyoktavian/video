...
const SourceMonitorContent = memo(function SourceMonitorContent({
  ...
  const [onlineMeta, setOnlineMeta] = useState<{ width: number; height: number; duration: number } | null>(null);
  ...
  const media = useMemo(() => {
    if (mediaId.startsWith('online:')) {
      try {
        const ep = JSON.parse(mediaId.slice(7));
        return {
          id: mediaId,
          fileName: ep.dramaTitle || ep.name || 'Online Episode',
          duration: onlineMeta?.duration || ep.duration || 0,
          fps: 30,
          width: onlineMeta?.width || ep.width || 1280,
          height: onlineMeta?.height || ep.height || 720,
          mimeType: 'video/mp4',
          audioCodec: 'mp4a.40.2', // Enables audio handling for online episodes
        } as any;
      } catch {
        return undefined;
      }
  ...
  useEffect(() => {
    ...
    // For online episodes, use the URL directly from the episode JSON
    if (mediaId.startsWith('online:')) {
      try {
        const ep = JSON.parse(mediaId.slice(7));
        if (!cancelled && ep.url) {
          ...
          // Preload metadata to get correct width, height, and duration
          ...
          video.onloadedmetadata = () => {
            if (!cancelled) {
              setOnlineMeta({
                ...
                duration: video.duration || ep.duration || 0,
              });
            }
          };
        }
