...
export function isVideoPoolAbortError(error: unknown): boolean {
  ...
  return (
    error.name === "AbortError" ||
    error.message.startsWith(VIDEO_POOL_ABORT_PREFIX)
  );
}
...
class SourceController {
  ...
  seekElement(
    ...
    const clampedTime = Math.max(
      0,
      Math.min(sourceTimeSeconds, duration - 0.001),
    );
    ...
    if (options?.fast && "fastSeek" in element) {
      (
        element as HTMLVideoElement & { fastSeek: (time: number) => void }
      ).fastSeek(clampedTime);
    } else {
...
export class VideoSourcePool {
  ...
  private pendingReleaseTimers: Map<string, ReturnType<typeof setTimeout>> =
    new Map();

  // Callbacks
  private onElementReady?: (
    sourceUrl: string,
    element: HTMLVideoElement,
  ) => void;
