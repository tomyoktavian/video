...
export function useTimelineDrag(
  ...
  const handleDragStart = useCallback(
    (e: React.MouseEvent) => {
      ...
      if (!isInSelection) {
        // Shift+mousedown: add to existing selection before starting drag
        if (e.shiftKey) {
          const merged = linkedSelectionEnabled
      ...
      // When Shift+mousedown adds to selection, use the merged selection for drag
      const effectiveSelectedIds = (!isInSelection && e.shiftKey)
        ? useSelectionStore.getState().selectedItemIds
        : currentSelectedIds;
      const baseItemsToDrag = (isInSelection || (!isInSelection && e.shiftKey))
        ? (linkedSelectionEnabled ? expandSelectionWithLinkedItems(allItems, effectiveSelectedIds) : effectiveSelectedIds)
        : linkedIds;
