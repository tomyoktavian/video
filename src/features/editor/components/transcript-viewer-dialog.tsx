    ...
    <Dialog open={isOpen} onOpenChange={handleOpenChange}>
      <DialogContent className="max-w-xl max-h-[80vh] flex flex-col gap-3">
        <DialogHeader>
          ...
          {fileName && <DialogDescription className="truncate">{fileName}</DialogDescription>}
        </DialogHeader>
          ...
          <>
            ...
            <div className="flex items-center justify-between text-[11px] text-muted-foreground">
              <div className="flex items-center gap-3">
                <span>
                  Model: <span className="text-foreground">{transcript.model}</span>
                </span>
                {transcript.language && (
                  <span>
                    Lang: <span className="text-foreground">{transcript.language}</span>
                  </span>
                )}
                <span>
                  Segments: <span className="text-foreground">{transcript.segments.length}</span>
                </span>
              </div>
              <Button
                variant="ghost"
              ...
              <div className="flex-1 min-h-0 overflow-y-auto rounded-lg border border-border bg-secondary/20">
                <div className="divide-y divide-border/50">
                  {transcript.segments.map((seg: MediaTranscriptSegment, i: number) => (
                    <div
                      key={i}
                      className="flex gap-3 px-3 py-2 hover:bg-secondary/30 transition-colors"
                    >
